  ...
  try {
    switch (action) {
      case "session": {
        ...
        // For seller app, verify user has seller role or is a seller
        if (data.authenticated && data.user) {
          const isSeller =
            data.user.role === "seller" ||
            data.user.role === "admin" ||
            data.user.role === "super_admin" ||
            data.user.sellerId; // User has an associated seller account
          if (!isSeller) {
            return NextResponse.json(
              { authenticated: false, error: "Unauthorized - Seller access required" },
